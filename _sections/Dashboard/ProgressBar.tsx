...
const ProgressBar: React.FC<ProgressBarProps> = ({ status,endTime, width=33}):JSX.Element => {
	...
	return (<Progress trailColor={progressStatus === 'exception' ?`#f30000` : ''} percent={progressStatus === 'exception' ? 100: progressPercentage} status={progressStatus} format={()=>getProgressBarText(days)} type="circle" width={width}/>)
}
